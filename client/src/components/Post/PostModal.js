...
const ChooseWinner = ({ postId, player }) => {
  ...
  const clickHandler = (winner, loser) => {
    ...
    axios
      .post(
        `http://ec2-13-124-226-101.ap-northeast-2.compute.amazonaws.com/match/${postId}`,
        { winner, loser },
...
const DeleteBtns = () => {
  ...
  const confirmDelete = () => {
    const Authorization = `Bearer ${localStorage.getItem("token")}`;
    axios
      .delete(`http://ec2-13-124-226-101.ap-northeast-2.compute.amazonaws.com/doc/${postId}`, {
        headers: { Authorization },
      })
      .then((res) => {
        history.push("/main");
...
const ChangeStatusBtn = ({ status, setPostInfo }) => {
  ...
  const changeStatus = () => {
    ...
    if (status === "대기") {
      axios
        .put(
          `http://ec2-13-124-226-101.ap-northeast-2.compute.amazonaws.com/doc/${postId}`,
          { status: "완료" },
          { headers: { Authorization } }
        )
        .then((res) => {
