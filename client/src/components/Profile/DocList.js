...
export default function DocList({ userId, isMypage, nickname }) {
  ...
  const handleCreatedList = () => {
    setIsLoading(true);
    axios
      .get(`http://ec2-13-124-226-101.ap-northeast-2.compute.amazonaws.com/doc?hostId=${userId}`)
      .then((res) => {
  ...
  const handleLikeList = () => {
    setIsLoading(true);
    axios
      .get(`http://ec2-13-124-226-101.ap-northeast-2.compute.amazonaws.com/like/${userId}`)
      .then((res) => {
        setList(res.data.data);
        setTimeout(() => {
          setIsLoading(false);
  ...
  const handleProgressList = () => {
    setIsLoading(true);
    axios
      .get(`http://ec2-13-124-226-101.ap-northeast-2.compute.amazonaws.com/doc?guestId=${userId}`)
      .then((res) => {
        setList(res.data.data);
        setTimeout(() => {
          setIsLoading(false);
  ...
  const handleHistoryList = () => {
    setIsLoading(true);
    axios
      .get(`http://ec2-13-124-226-101.ap-northeast-2.compute.amazonaws.com/match/history?userId=${userId}`)
      .then((res) => {
