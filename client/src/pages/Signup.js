...
export default function Signup() {
  ...
  useEffect(() => {
    setMessage({
      ...
      checkPW:
        signupInfo.checkPW.length >= signupInfo.password.length && signupInfo.password.length
    ...
    setValidation({
      ...
      username: isUsername(signupInfo.username) && signupInfo.username.length >= 2 && signupInfo.username.length < 8,
  ...
  return (
    <div>
      <div className='inputContainer'>
        <input type='email' placeholder='E-mail' onBlur={handleOnblurEmail('email')} onChange={handleInputValue('email')} onKeyPress={handleKeyPress} />
        {message.email === '이메일 주소를 입력해주세요' ? <div>{message.email}</div> : message.email === '사용 가능한 이메일입니다' ? <div>{message.email}</div> : <div>{message.email}</div>}
        <input type='username' placeholder='닉네임' onBlur={handleOnblurName('username')} onChange={handleInputValue('username')} onKeyPress={handleKeyPress} />
        {message.username === '한글만 입력해주세요' ? <div>{message.username}</div> : <div>{message.username}</div>}
        <input type='password' placeholder='비밀번호' onChange={handleInputValue('password')} onKeyPress={handleKeyPress} />
        {message.password === '비밀번호는 8자리 이상, 숫자, 문자, 특수문자가 포함되어야 합니다' ? (
        ...
        <input type='password' placeholder='비밀번호확인' onChange={handleInputValue('checkPW')} onKeyPress={handleKeyPress} />
        {message.checkPW === '비밀번호가 불일치합니다' ? <div>{message.checkPW}</div> : <div>{message.checkPW}</div>}
      </div>
      <div className='btnContainer'>
        <button onClick={() => history.push('/main')}>돌아가기</button>
        {isValid ? (
          <button style={{ color: 'green' }} type='submit' onClick={handleSignup}>
            가입하기
          </button>
        ) : (
          <button>가입하기</button>
