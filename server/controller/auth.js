...
export async function me(req, res) {
  ...
  return res.status(200).json({ userId: user.id, nickname: user.nickname, img: user.img });
}
...
export async function signup(req, res) {
  ...
  if (email === undefined || nickname === undefined || password === undefined || type === undefined) {
...
export async function kakaoLogin(req, res) {
  ...
  return res.redirect(`https://kauth.kakao.com/oauth/authorize?client_id=42184b4ebbf71c527914d5cf6269aae0&redirect_uri=http://localhost:8080/auth/kakao/callback&&response_type=code`);
}
...
export async function kakaoCallback(req, res) {
  ...
  try {
    ...
    res.send('이제 받은 유저정보 유저테이블에 해당 id가 없으면 해당 id로 유저테이블에 유저생성, 쿠키주기, 있으면 있는걸로 꺼내서 쿠키주기');
  } catch (error) {
    console.error(error);
